...
class RetrievalQueryRoutingAgent:
    ...
    def extract_entities(self, query: str, session_id: Optional[str] = None) -> Dict[str, List[str]]:
        """Extract drug names and intent from query using Gemini, with short Redis-backed history context."""
        history_context = ""
        if session_id and self.session_agent:
            try:
                ...
                profile = self.session_agent.get_profile(session_id)
                if profile:
                    p_age = profile.get("age", "-")
                    p_gender = profile.get("gender", "-")
                    p_cond = profile.get("conditions", "-")
                    profile_text = f"Age: {p_age}, Gender: {p_gender}, Conditions: {p_cond}"
                    history_context += f"Patient profile: {profile_text}\n"
            except Exception as e:
                logger.warning(f"Failed to fetch history for entity extraction: {e}")
...
class AnswerGenerationAgent:
    ...
    def generate_final_response(self, query: str, session_id: str, retrievals: List[Dict]) -> Dict[str, Any]:
        """Generate final structured response using Gemini, pulling recent history from Redis."""
        if not retrievals:
            return {
        ...
        # Fetch recent history and patient profile
        history = []
        profile = {}
        if session_id and self.session_agent:
            try:
                history = self.session_agent.get_history(session_id)
                profile = self.session_agent.get_profile(session_id) or {}
            except Exception as e:
                logger.warning(f"Failed to fetch history/profile for answer generation: {e}")
        ...
        # Patient profile string (optional)
        profile_text = ""
        if profile:
            p_age = profile.get("age", "-")
            p_gender = profile.get("gender", "-")
            p_cond = profile.get("conditions", "-")
            profile_text = f"\nPatient Profile: Age={p_age}, Gender={p_gender}, Conditions={p_cond}\n"
    ...
    def generate_smalltalk_response(self, query: str, session_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Use Gemini to handle small-talk/basic usage or off-topic queries when no document context is available."""
        q_raw = (query or "").strip()
        q = q_raw.lower()
        if not q:
            return None

        # Strict special-case: name must be GitHub Copilot
        if any(phrase in q for phrase in [
            "your name", "who are you", "what is your name", "whats your name", "what's your name"
        ]):
            return {
                "short_answer": "GitHub Copilot",
                "confidence_score": 1.0,
                "citations": [],
                "reasoning": "Name response per system requirement."
            }

        # Pull a tiny bit of context for continuity
        history_text = ""
        if session_id and self.session_agent:
            try:
                hist = self.session_agent.get_history(session_id)
                recent = [m for m in hist if m.get("role") != "system"][-3:]
                if recent:
                    history_text = "\n".join([f"{m['role']}: {m['content']}" for m in recent])
            except Exception:
                history_text = ""
        ...
        prompt = f"""
        You are a concise, friendly assistant for a drug-information chatbot.
        The user's message may be a greeting, a simple usage question, or off-topic.
        - If it's greeting/thanks/farewell, reply politely in 1 short sentence.
        - If it's about how to use the app, briefly explain (add PDFs to 'pdfs', click 'Ingest PDFs', then ask a drug question).
        - If it's off-topic, respond politely that you focus on drug information and ask them to name a drug and topic (e.g., dosing, side effects).
        - Do not include citations. Keep it under 25 words.
        ...
        try:
            ...
            return {
                ...
                "citations": [],
                "reasoning": "Small-talk/basic response generated via Gemini."
            }
        except Exception:
            return None
...
class SessionManagementAgent:
    ...
    def get_profile(self, session_id: str) -> Dict[str, Any]:
        ...
        if self.use_redis:
            ...
            return st.session_state.get(f"profile_{session_id}", {})

    def update_profile(self, session_id: str, profile: Dict[str, Any]):
        key = f"{session_id}:profile"
        # Sanitize fields
        allowed = {"age", "gender", "conditions"}
        clean = {k: v for k, v in (profile or {}).items() if k in allowed}

        if self.use_redis:
            try:
                self.redis_client.set(key, json.dumps(clean))
            except:
                pass
        else:
            st.session_state[f"profile_{session_id}"] = clean
...
class DrugChatbotOrchestrator:
    ...
    def ingest_pdfs(self):
        # Discover and ingest all PDFs in the folder, inferring a normalized drug name per file
        pdf_paths = sorted(PDF_DIR.glob("*.pdf"))
        if not pdf_paths:
            st.warning(f"⚠️ No PDFs found in {PDF_DIR}")
            return

        for pdf_path in pdf_paths:
            filename = pdf_path.name
            drug_name = self._infer_drug_name(filename)
            try:
                ...
                st.success(f"✅ Ingested {filename} as {drug_name}")
            except Exception as e:
                st.error(f"❌ Failed to ingest {filename}: {e}")

    def _infer_drug_name(self, filename: str) -> str:
        """Infer a consistent drug name from a filename. Uses keyword mapping then fallback cleanup."""
        f = filename.lower()
        # Common brand/generic keywords mapped to display names used in metadata and filtering
        keyword_map = {
            "humira": "Humira",
            "rinvoq": "Rinvoq",
            "skyrizi": "Skyrizi",
            ...
            "ustekinumab-aekn": "Ustekinumab-aekn",
        ...
        # Fallback: clean the stem and title-case
        stem = Path(filename).stem
        cleaned = re.sub(r"[\(\)\[\]]", " ", stem)
        cleaned = re.sub(r"[_\-]+", " ", cleaned)
        cleaned = re.sub(r"\b(pi|medguide|lbl|label|epar|auspar|product|information|en|us|eu)\b", " ", cleaned, flags=re.I)
    ...
    def _expand_query(self, query: str, entities: Dict[str, Any]) -> str:
        """Expand query with simple domain synonyms to broaden retrieval when context is scarce."""
        ql = (query or "").lower()
        extras = []
        # Oncology
        if "cancer" in ql or "malignan" in ql or "oncolog" in ql:
            extras += ["malignancy", "malignancies", "oncology", "tumor", "tumors", "neoplasm", "neoplasms", "lymphoma"]
        # Pregnancy
        if "pregnan" in ql:
            extras += ["pregnancy", "pregnant", "gestation", "lactation", "breastfeeding"]
        # Renal/hepatic
        if "renal" in ql or "kidney" in ql:
            extras += ["renal impairment", "kidney impairment", "renal dosing", "dose adjustment"]
        if "hepatic" in ql or "liver" in ql:
            extras += ["hepatic impairment", "liver impairment", "hepatic dosing", "dose adjustment"]
        # Safety
        if "side effect" in ql or "adverse" in ql:
            extras += ["adverse reactions", "safety", "warnings", "precautions", "boxed warning", "contraindications"]
        if not extras:
            return query
        return f"{query} (related terms: {'; '.join(sorted(set(extras)) )})"

    def process_query(self, query: str, session_id: str) -> Dict[str, Any]:
        # Crew AI tasks (for UI transparency only)
        entities_task = Task(
        ...
        self.crew.tasks.extend([entities_task, retrieval_task, reasoning_task, answer_task, session_task])

        # Step 1: Entity Recognition (with Redis-backed context)
        ...
        if entities.get("drugs"):
            normalized = list({self._infer_drug_name(d) for d in entities["drugs"] if isinstance(d, str) and d.strip()})
            if normalized:
                filter_metadata = {"drug": {"$in": normalized}}

        logger.info(f"Calling agent: {self.crew_agents['retrieval'].role}")
        retrievals = self.retrieval_agent.vector_search(query, top_k=8, filter_metadata=filter_metadata)
        ...
        if not has_sufficient_info:
            # Broadened search: expand query semantics and remove drug filter
            expanded_query = self._expand_query(query, entities)
            if expanded_query != query:
                logger.info("Insufficient info; retrying with broadened search across all documents.")
                retrievals2 = self.retrieval_agent.vector_search(expanded_query, top_k=8, filter_metadata=None)
                filtered_retrievals2 = self.reasoning_agent.assess_chunk_relevance(expanded_query, retrievals2)
                if self.reasoning_agent.check_relationships_exist(filtered_retrievals2, expanded_query):
                    logger.info(f"Calling agent: {self.crew_agents['answer'].role} (broadened)")
                    response = self.answer_agent.generate_final_response(query, session_id, filtered_retrievals2[:4])
                    response["entities_found"] = entities
                    # annotate reasoning that broadened search was used
                    response["reasoning"] = (response.get("reasoning", "") + " | Used broadened search.").strip()
                    return response
            # Small-talk/app-help fallback
            smalltalk = self.answer_agent.generate_smalltalk_response(query, session_id)
            if smalltalk:
                smalltalk["entities_found"] = entities
                return smalltalk
            # Default insufficient info message
            return {
                "short_answer": "I don't have sufficient relevant information to answer that. Try asking about a specific drug (e.g., dosing, side effects, interactions).",
                "confidence_score": 0.0,
                "citations": [],
        ...
        # Step 5: Generate Final Answer (with Redis-backed context)
...
def main():
    ...
    with st.sidebar:
        ...
        existing = orchestrator.session_agent.get_profile(st.session_state.session_id)
        age = st.number_input("Age", min_value=0, max_value=120, value=int(existing.get("age", 0)) if isinstance(existing.get("age"), (int, float)) else 0)
        gender = st.selectbox("Gender", ["", "Female", "Male", "Non-binary", "Other"], index=( ["", "Female", "Male", "Non-binary", "Other"].index(existing.get("gender")) if existing.get("gender") in ["", "Female", "Male", "Non-binary", "Other"] else 0 ))
        conditions = st.text_input("Conditions (comma-separated)", value=str(existing.get("conditions", "")))
        if st.button("Save Profile"):
            orchestrator.session_agent.update_profile(
                st.session_state.session_id,
                {"age": int(age) if age else None, "gender": gender or None, "conditions": conditions.strip() or None}
            )
    ...
    if query := st.chat_input("Ask about drug information (e.g., 'What are the side effects of Humira?')"):
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Processing your query..."):
                ...
                filter_metadata = None
                if entities.get("drugs"):
                    normalized = list({orchestrator._infer_drug_name(d) for d in entities["drugs"] if isinstance(d, str) and d.strip()})
                    if normalized:
                        drug_filter = {"drug": {"$in": normalized}}
                ...
                if not has_sufficient_info:
                    # Broadened search: expand query and remove filters
                    expanded_query = orchestrator._expand_query(query, entities)
                    if expanded_query != query:
                        agent_status.info("Current running agent: Retrieval Agent (broadened)")
                        retrievals = orchestrator.retrieval_agent.vector_search(expanded_query, top_k=8, filter_metadata=None)
                        agent_status.info("Current running agent: Reasoning Agent (broadened)")
                        filtered_retrievals = orchestrator.reasoning_agent.assess_chunk_relevance(expanded_query, retrievals)
                        has_sufficient_info = orchestrator.reasoning_agent.check_relationships_exist(filtered_retrievals, expanded_query)

                if not has_sufficient_info:
                    smalltalk = orchestrator.answer_agent.generate_smalltalk_response(query, st.session_state.session_id)
                    if smalltalk:
                        smalltalk["entities_found"] = entities
                        response = smalltalk
                    else:
